...
const schema = z.object({
  fname: z.string().nonempty("Please type your first name"),
...
const BookingForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow">
      <h2 className="text-2xl font-bold mb-6">Finalize your Booking</h2>

      <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block mb-1 font-medium">First name</label>
          <input type="fname" {...register("fname" as const)} className="w-full p-3 border border-gray-300 rounded-md" />
          {errors.fname && <p className="text-red-500 text-sm mt-1">{errors.fname.message}</p>}
        </div>

        <div>
          <label className="block mb-1 font-medium">Last name</label>
          <input type="lname" {...register("lname" as const)} className="w-full p-3 border border-gray-300 rounded-md" />
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="email" {...register("email" as const)} className="w-full p-3 border border-gray-300 rounded-md" />
          {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email.message}</p>}
        </div>

        <div>
          <label className="block mb-1 font-medium">Phone Number</label>
          <input type="tel" {...register("phone" as const)} className="w-full p-3 border border-gray-300 rounded-md" />
      ...
      <div className="mb-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          {imagePreviews.map((src, idx) => (
            <img key={idx} src={src} alt="Preview" className="rounded-lg shadow h-32 object-cover" />
          ))}
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <textarea {...register("notes")} rows={4} className="w-full p-3 border border-gray-300 rounded-md" />
      </div>
